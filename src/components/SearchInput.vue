...
const handleSearch = () => {
  ...
  searchTerm.timeout = setTimeout(async () => {
    ...
    const res = await fetch(
      `http://api.weatherapi.com/v1/search.json?key=de68334555a64cf2b27112031240603&q=${searchTerm.query}`)
...
const getWeather = async (id) => {
    const res = await fetch(
      `http://api.weatherapi.com/v1/forecast.json?key=de68334555a64cf2b27112031240603&q=id:${id}&days=3&aqi=no&alerts=no`)
